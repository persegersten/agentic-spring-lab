...
export function RoundPlayback({round,players,onVehicles,onEvent,onFinished}:{round:PublicRound;players:Player[];onVehicles:(v:Vehicle[])=>void;onEvent:(event?:RoundEvent)=>void;onFinished:()=>void}){
 ...
 useEffect(()=>{
  ...
  for(const event of round.playback.slice(0,eventIndex)){
   ...
   if(vehicle&&['MOVE','TURN','RAM','PUSH'].includes(event.type)){vehicle.x=event.newPosition.x;vehicle.y=event.newPosition.y;vehicle.direction=event.newDirection}
   if(vehicle&&event.type==='DAMAGE')vehicle.damage=event.newDamage
  }
  onVehicles(vehicles)
 },[eventIndex,round.initialVehicles,round.playback,onVehicles])
 useEffect(()=>onEvent(round.playback[eventIndex-1]),[eventIndex,onEvent,round.playback])
