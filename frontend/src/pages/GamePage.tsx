...
export function GamePage() {
  ...
  if (view && session) {
    ...
    return <main className="game-page"><header><div><p className="eyebrow">Wreckage control deck</p><h1>WRECKAGE</h1></div><span className="game-code">Spel {view.id.slice(0, 8)}</span></header>{error && <p className="error" role="alert">{error}</p>}<div className="game-layout"><section><GameBoard board={view.board} vehicles={vehicles} players={view.players} /></section><div className="sidebar">{!round && <section className="panel"><h2>Spelare anslutna</h2><p>{view.players.map(p => p.name).join(', ')}</p><p>{view.players.length} / {view.configuration.maxPlayers} spelare</p><LobbyCountdown joinDeadline={view.joinDeadline} /></section>}{round && <RoundStatus round={round} players={view.players} />} {round?.phase === 'PLANNING' && <CommandHand hand={view.round?.hand ?? []} locked={round.ready[view.playerId]} onSubmit={async orders => run(async () => setView(await submitProgram(session, orders)))} />} {round?.phase === 'PLAYBACK' && <RoundPlayback round={round} players={view.players} onVehicles={setVehicles} onFinished={() => setPlaybackDone(true)} />} {playbackDone && <button onClick={() => void run(async () => { setPlaybackDone(false); setView(await startRound(session)) })}>Starta nästa runda</button>}</div></div></main>
  }
