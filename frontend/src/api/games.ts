...
export async function saveProgramDraft(session:PlayerSession,orders:MovementOrder[]):Promise<PlayerGame>{return json(await fetch(`/games/${session.gameId}/rounds/current/program`,{method:'PUT',headers:{...auth(session),'Content-Type':'application/json'},body:JSON.stringify({orders})}),'Kunde inte spara kortordningen')}
