...
class RoundEntity {
    ...
    private static String encodePlayback(List<RoundEvent> events) { return events.stream().map(e -> String.join(",",
            ...
            e.oldDirection().name(),e.newDirection().name())).collect(java.util.stream.Collectors.joining("#")); }
    private static List<RoundEvent> decodePlayback(String value, Map<UUID,Vehicle> vehicles) { if(value.isBlank()) return List.of(); var result=new ArrayList<RoundEvent>(); for(String row:value.split("#")){String[] p=row.split(","); result.add(new RoundEvent(Integer.parseInt(p[0]),RoundEventType.valueOf(p[1]),UUID.fromString(p[2]),UUID.fromString(p[3]),new Position(Integer.parseInt(p[4]),Integer.parseInt(p[5])),new Position(Integer.parseInt(p[6]),Integer.parseInt(p[7])),Direction.valueOf(p[8]),Direction.valueOf(p[9])));} return result; }
