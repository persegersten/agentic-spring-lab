...
public record VehicleResponse(UUID id, UUID playerId, int x, int y, Direction direction, int damage) {
    static VehicleResponse from(VehicleState s) { return new VehicleResponse(s.vehicle().id(),s.vehicle().playerId(),s.position().x(),s.position().y(),s.orientation(),s.damage()); }
}
