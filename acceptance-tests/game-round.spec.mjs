...
test('players lock private programs and watch the same three-card playback', async ({ browser }) => {
  await withPlayerPages(browser, ['alice', 'bob'], async ({ alice, bob }) => {
    ...
    await expect(alice.getByText('Alice, Bob', { exact: true })).toBeVisible()
    await alice.getByRole('button', { name: 'Starta första rundan', exact: true }).click()

    const programs = {}
    for (const [name, page] of [['Alice', alice], ['Bob', bob]]) {
      await expect(page.getByRole('heading', { name: 'Programmering', exact: true })).toBeVisible()
    ...
    await expect(bob.getByRole('heading', { name: 'Programmering', exact: true })).toBeVisible()
